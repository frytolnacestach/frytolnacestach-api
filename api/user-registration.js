...
app.use(cors()); // Přidání middleware pro CORS
app.use(express.json()); // Povolit parsování JSON dat
...
router.post("/", async (req, res) => {
  ...
  if (existingUser.length > 0) {
    return res
      .status(400)
      .send("Uživatel s touto e-mailovou adresou již existuje.");
  }
  ...
  try {
    ...
    return res.status(201).send("Účet vytvořen");
  } catch (error) {
    console.error(error);
    return res.status(500).send("Server error");
  }
